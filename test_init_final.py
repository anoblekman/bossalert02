				...
				if fixed_bossTime[i] <= priv0 and fixed_bossTime[i] > priv:
					if basicSetting[3] != '0':
						if fixed_bossFlag0[i] == False:
                            ...
                            # fixed_bossTime_str = fixed_bossTime[i].strftime('%H:%M:%S')
                            # await client.get_channel(channel).send("```" + fixed_bossData[i][0] + ' ' + basicSetting[3] + '분 전 ' + fixed_bossData[i][3] +" [" +  fixed_bossTime_str + "]```", tts=False)
				...
				if fixed_bossTime[i] <= priv and fixed_bossTime[i] > now:
					if basicSetting[1] != '0' :
						if fixed_bossFlag[i] == False:
                            ...
                            # fixed_bossTime_str = fixed_bossTime[i].strftime('%H:%M:%S')
                            # await client.get_channel(channel).send("```" + fixed_bossData[i][0] + ' ' + basicSetting[1] + '분 전 ' + fixed_bossData[i][3] +" [" +  fixed_bossTime_str + "]```", tts=False)
